...
export async function unlockIntegration(token, confirm_text) {
  return parse(await fetch(`${API_BASE}/trading/unlock`, {
    method: "POST",
    headers: { "Content-Type": "application/json", ...authHeaders(token) },
...
export async function manualConfirmTrade(token, payload) {
  return parse(await fetch(`${API_BASE}/trading/manual-confirm`, {
    method: "POST",
    headers: { "Content-Type": "application/json", ...authHeaders(token) },
    body: JSON.stringify(payload)
  }));
