    ...
    with db() as conn:
        ...
        settings = conn.execute(
            ...
            SELECT paper_mode, strategy_mode, execution_provider, max_position_usd,
                   daily_loss_limit_usd, auto_trading_enabled
...
def simulate_trade(user_id: str) -> dict:
    ...
    exec_result = execute(trade["symbol"], trade["size_usd"], status["execution_provider"], status["paper_mode"])
    ...
    with db() as conn:
        conn.execute(
            ...
            (
                ...
                trade["strategy_name"],
                trade["regime"],
                "paper" if status["paper_mode"] else "integration",
                _now(),
            ),
        )

        state = conn.execute(
            "SELECT daily_pnl_usd, total_pnl_usd, trades_today FROM trading_state WHERE user_id = ?",
            (user_id,),
        ).fetchone()
        ...
        max_drawdown = round(max(abs(min(0.0, daily)), status["max_drawdown_pct"]), 2)
        ...
        conn.execute(
            ...
            (
                ...
                win_rate,
                max_drawdown,
                f"{trade['strategy_name']} | regime={trade['regime']} | rl={rl_score} | provider={exec_result['provider']}",
                _now(),
                user_id,
            ),
        )
