...
def ensure_fund_state():
    for key, default_val in {
        "stable": getattr(rt, "FUND_STABLE_BASE", 0.40),
...
def update_fund_allocator(force=False):
    ...
    raw_scores = {}
    total_score = 0.0
    for strat in ["stable", "sniper", "smart", "momentum", "explore"]:
        ...
        if trades < rt.FUND_MIN_TRADES:
            base_prior = {"stable": 1.15, "sniper": 1.00, "smart": 1.05, "momentum": 1.05, "explore": 0.55}.get(strat, 1.0)
            score = base_prior
        else:
...
def fund_multiplier(strategy: str) -> float:
    strategy = strategy_bucket_from_mode(strategy)
    alloc = sf(rt.FUND_ALLOCATOR.get(strategy, rt.FUND_ALLOCATOR.get("momentum", 0.25)), 0.25)
    if alloc >= 0.45:
        return 1.35
    if alloc >= 0.35:
        return 1.18
